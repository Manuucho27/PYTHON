...
def calculadoraBasica():
    ...
    while True:
        try:
            ...
            resultado = realizarOperacion(num1, num2, op)
...
def realizarOperacion(num1, num2, op):
    if op == "+":
        return num1 + num2
    elif op == "-":
        return num1 - num2
...
def tablaMultiplicar():
    ...
    try:
        ...
        print(f"Tabla de multiplicar (avanzado) del {n}:")
        resCogidos = tabla(n)
        for j in range(1, 11):
            print(f"{n} x {j} = {resCogidos[j-1]}")

        print(f"Tabla de multiplicar (simple) del {n}:")
        for i in range(1, 11):
            print(f"{n} x {i} = {tablaSimple(n, i)}")
    except ValueError:
...
def comprobarPrimo():
    ...
    try:
        ...
        if numero == 1:
            ...
            print(f"{numero} es primo." if esPrimo(numero) else f"{numero} no es primo.")
    except ValueError as e:
        print("Error:", e)
    print("")
...
def analisisNotas():
    ...
    if not notas:
        ...
        minima = notaMinima(notas)

        print(f"\nTotal de alumnos: {len(notas)}")
        print(f"Aprobados: {aprobados}")
        print(f"Nota media: {media.__round__(2)}")
...
def bloque1():
    while True:
        ...
        match opcion:
            ...
            case "3": adivinaNumero()
            case "4": comprobarPrimo()
            case "5": primosHastaNum()
            case "6": analisisNotas()
            case "0":
...
def inventarioInteligente():
    ...
    while True:
        ...
        match opcion:
            case "1": agregar_producto(inventario)
            case "2": vender_producto(inventario)
            case "3": mostrar_inventario(inventario)
            case "4": eliminar_producto(inventario)
            case "5":
...
def inventarioSimple():
    ...
    while True:
        ...
        match opcion:
            case "1":
                manzanas, platanos, peras = agregarStockSimple(manzanas, platanos, peras)
            case "2":
                manzanas, platanos, peras = venderSimple(manzanas, platanos, peras)
            case "3":
                print(f"\nINVENTARIO ACTUAL:\n- Manzanas: {manzanas}\n- Plátanos: {platanos}\n- Peras: {peras}")
            case "4":
                print("Volviendo al menú del bloque...")
...
def agregarStockSimple(manzanas, platanos, peras):
    ...
    try:
        ...
        print("Error:", e)
    return manzanas, platanos, peras


def venderSimple(manzanas, platanos, peras):
    print("\nProductos disponibles: Manzanas, Plátanos, Peras")
...
def inversionDinero():
    ...
    try:
        ...
        total = calcularInversion(dineroEnBanco, interesAnual, anios)
        print(f"\nEl dinero final será: {total:.2f}€ tras {anios} años con {interesAnual}% de interés anual.")
    except ValueError as e:
        print("Error:", e)


def calcularInversion(capital, interesAnual, anios):
...
def menuRestaurante():
    ...
    try:
        comida = plato[int(input("\n--- COMIDAS ---\n1. Macarrones\n2. Sushi\n3. Filete de pollo\nElige: "))]
        bebidaSel = bebida[int(input("\n--- BEBIDAS ---\n1. Fanta\n2. Coca-Cola\n3. Cerveza\nElige: "))]
        comp = complemento[int(input("\n--- COMPLEMENTOS ---\n1. Nuggets\n2. Gyozas\n3. Mini pizza\nElige: "))]
...
def bloque2():
    while True:
        ...
        match opcion:
            case "1": inventarioInteligente()
            case "2": inventarioSimple()
            case "3": inversionDinero()
